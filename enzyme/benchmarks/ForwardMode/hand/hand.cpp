  ...
  for (i = lhs->nrows - 1; i > -1; --i)
    for (k = rhs->ncols - 1; k > -1; --k) {
      for (j = lhs->ncols - 1; j > 0; --j) {
        lhsb->data[i + j * lhs->nrows] =
            lhsb->data[i + j * lhs->nrows] +
            rhs->data[j + k * rhs->nrows] * outb->data[i + k * out->nrows];
        rhsb->data[j + k * rhs->nrows] =
            rhsb->data[j + k * rhs->nrows] +
            lhs->data[i + j * lhs->nrows] * outb->data[i + k * out->nrows];
      }
      lhsb->data[i + 0 * lhs->nrows] =
          lhsb->data[i + 0 * lhs->nrows] +
          rhs->data[0 + k * rhs->nrows] * outb->data[i + k * out->nrows];
      rhsb->data[0 + k * rhs->nrows] =
          rhsb->data[0 + k * rhs->nrows] +
          lhs->data[i + 0 * lhs->nrows] * outb->data[i + k * out->nrows];
...
void set_block_b(Matrix *mat, Matrix_diff *matb, int row_off, int col_off,
  ...
  for (i_col = block->ncols - 1; i_col > -1; --i_col)
    for (i_row = block->nrows - 1; i_row > -1; --i_row) {
      blockb->data[i_row + i_col * block->nrows] =
          blockb->data[i_row + i_col * block->nrows] +
          matb->data[i_row + row_off + (i_col + col_off) * mat->nrows];
...
*/
// copies one matrix to another
void copy_b(Matrix *dst, Matrix_diff *dstb, const Matrix *src,
            Matrix_diff *srcb) {
  int chunklength;
  int ii1;
  void *diffchunkold;
...
========================================================================
                              MAIN LOGIC
========================================================================*/
void angle_axis_to_rotation_matrix_b(const double *angle_axis,
                                     double *angle_axisb, Matrix *R,
                                     Matrix_diff *Rb) {
  double norm;
  double normb;
  double result1;
  ...
  if (norm < 0.0001) {
    ...
    normb =
        cos(norm) * sb - sin(norm) * cb - angle_axis[2] * zb / (norm * norm) -
...
/*
  Differentiation of apply_global_transform in reverse (adjoint) mode:
   ...
   Plus diff mem management of: alloc(*mat).data:in-out alloc(*mat).data:in-out
                ...
                *positions.data:in
*/
void apply_global_transform_b(const Matrix *pose_params,
                              Matrix_diff *pose_paramsb, Matrix *positions,
                              Matrix_diff *positionsb) {
  int i, j;
  Matrix *R;
  Matrix_diff *Rb;
  int branch;
  ...
  for (i = 0; i < 3; ++i)
    for (j = 0; j < 3; ++j) {
      ...
      R->data[j + i * R->nrows] = R->data[j + i * R->nrows] *
                                  pose_params->data[i + 1 * pose_params->nrows];
    }
  Matrix *tmp;
  Matrix_diff *tmpb;
  ...
  for (j = positions->ncols - 1; j > -1; --j)
    for (i = positions->nrows - 1; i > -1; --i) {
      tmpb->data[i + j * tmp->nrows] =
          tmpb->data[i + j * tmp->nrows] +
          positionsb->data[i + j * positions->nrows];
      pose_paramsb->data[i + 2 * pose_params->nrows] =
          pose_paramsb->data[i + 2 * pose_params->nrows] +
          positionsb->data[i + j * positions->nrows];
  ...
  for (i = 2; i > -1; --i)
    for (j = 2; j > -1; --j) {
      ...
      pose_paramsb->data[i + 1 * pose_params->nrows] =
          pose_paramsb->data[i + 1 * pose_params->nrows] +
          R->data[j + i * R->nrows] * Rb->data[j + i * R->nrows];
      Rb->data[j + i * R->nrows] =
          pose_params->data[i + 1 * pose_params->nrows] *
          Rb->data[j + i * R->nrows];
    }
  angle_axis_to_rotation_matrix_b(pose_params->data, pose_paramsb->data, R, Rb);
...
/*
  Differentiation of relatives_to_absolutes in reverse (adjoint) mode:
   ...
   Plus diff mem management of: absolutes:in *absolutes.data:out
                relatives:in
*/
void relatives_to_absolutes_b(int count, const Matrix *relatives,
                              Matrix_diff *relativesb, const int *parents,
                              Matrix *absolutes, Matrix_diff *absolutesb) {
  int i;
  int branch;
  for (i = 0; i < count; ++i)
    if (parents[i] == -1) {
      ...
      mat_mult_c(&(absolutes[parents[i]]), &(relatives[i]), &(absolutes[i]));
      pushControl1b(0);
    }
  for (i = count - 1; i > -1; --i) {
    popControl1b(&branch);
...
void relatives_to_absolutes_c(int count, const Matrix *relatives,
                              const int *parents, Matrix *absolutes) {
  int i;
  for (i = 0; i < count; ++i)
    if (parents[i] == -1)
...
void get_posed_relatives_b(int bone_count, const Matrix *base_relatives,
                           ...
                           const Matrix *pose_params, Matrix_diff *pose_paramsb,
                           Matrix *relatives, Matrix_diff *relativesb) {
  int i;
  int offset = 3;
  Matrix *tr;
  Matrix_diff *trb;
...
void get_posed_relatives_c(int bone_count, const Matrix *base_relatives,
  ...
  Differentiation of get_skinned_vertex_positions in reverse (adjoint) mode:
   ...
   Plus diff mem management of: alloc(*mat).data:in-out alloc(*mat).data:in-out
                ...
                *base_positions.data:in positions:in *positions.data:in-out
                inverse_base_absolutes:in
*/
void get_skinned_vertex_positions_b(
    int bone_count, const Matrix *base_relatives, const int *parents,
    const Matrix *inverse_base_absolutes, Matrix_diff *inverse_base_absolutesb,
    const Matrix *base_positions, Matrix_diff *base_positionsb,
    const Matrix *weights, int is_mirrored, const Matrix *pose_params,
    Matrix_diff *pose_paramsb, Matrix *positions, Matrix_diff *positionsb,
    int apply_global) {
  int i;
  Matrix *relatives;
  ...
  for (i = 0; i < bone_count; ++i) {
    ...
    if (absolutes[i].data) {
      ...
      pushControl1b(0);
    mat_mult_c(&(absolutes[i]), &(inverse_base_absolutes[i]), &(transforms[i]));
  }
  // Transform vertices by necessary transforms-> + apply skinning
  ...
  if (is_mirrored) {
    for (i = 0; i < positions->ncols; ++i)
      positions->data[0 + i * positions->nrows] =
          positions->data[0 + i * positions->nrows] * -1;
    pushControl1b(0);
  } else
    pushControl1b(1);
  ...
  popControl1b(&branch);
  if (branch == 0)
    apply_global_transform_b(pose_params, pose_paramsb, positions, positionsb);
  popControl1b(&branch);
  if (branch == 0)
    for (i = positions->ncols - 1; i > -1; --i)
      positionsb->data[0 + i * positions->nrows] =
          -positionsb->data[0 + i * positions->nrows];
  for (i_bone = bone_count - 1; i_bone > -1; --i_bone) {
    ...
    *base_positionsb->data = 0.0;
    mat_mult_b(&(transforms[i_bone]), &(transformsb[i_bone]), base_positions,
               base_positionsb, curr_positions, curr_positionsb);
  }
  ...
  for (i = bone_count - 1; i > -1; --i) {
    ...
    *inverse_base_absolutesb->data = 0.0;
    mat_mult_b(&(absolutes[i]), &(absolutesb[i]), &(inverse_base_absolutes[i]),
               &(inverse_base_absolutesb[i]), &(transforms[i]),
               &(transformsb[i]));
  }
  popControl1b(&branch);
  if (branch == 1)
...
void get_skinned_vertex_positions_c(
    int bone_count, const Matrix *base_relatives, const int *parents,
    const Matrix *inverse_base_absolutes, const Matrix *base_positions,
    const Matrix *weights, int is_mirrored, const Matrix *pose_params,
    Matrix *positions, int apply_global) {
  int i;
  Matrix *relatives;
  relatives = get_matrix_array_c(bone_count);
  Matrix *absolutes;
...
//%       similarly: index, middle, ring, pinky
//%       end) forearm
void to_pose_params_b(int count, const double *theta, double *thetab,
  ...
  for (i_finger = n_fingers - 1; i_finger > -1; --i_finger) {
    ...
    for (i = 4; i > 1; --i) {
      ...
      if (branch == 0) {
        ...
        thetab[i_theta] =
            thetab[i_theta] +
            pose_paramsb->data[1 + i_pose_params * pose_params->nrows];
        pose_paramsb->data[1 + i_pose_params * pose_params->nrows] = 0.0;
      }
      --i_theta;
      thetab[i_theta] =
          thetab[i_theta] +
          pose_paramsb->data[0 + i_pose_params * pose_params->nrows];
      pose_paramsb->data[0 + i_pose_params * pose_params->nrows] = 0.0;
    }
  }
  for (i = pose_params->nrows - 1; i > -1; --i) {
    thetab[i + 3] =
        thetab[i + 3] + pose_paramsb->data[i + 2 * pose_params->nrows];
...
void hand_objective_b(const double *theta, double *thetab, int bone_count,
                      ...
                      const Triangle *triangles, int is_mirrored,
                      int corresp_count, const int *correspondences,
                      Matrix *points, double *err, double *errb) {
  ...
  if (pose_params->data) {
    ...
    pushControl1b(0);
  pushPointer8(pose_params->data);
  get_skinned_vertex_positions_c(
      bone_count, base_relatives, parents, inverse_base_absolutes,
      base_positions, weights, is_mirrored, pose_params, vertex_positions, 1);
  int i, j;
  ...
  get_skinned_vertex_positions_b(
      bone_count, base_relatives, parents, inverse_base_absolutes,
      inverse_base_absolutesb, base_positions, base_positionsb, weights,
      is_mirrored, pose_params, pose_paramsb, vertex_positions,
      vertex_positionsb, 1);
