...
#define HAND_XYZ_TO_ROTATIONAL_PARAMETERIZATION                                \
  { 0, 2, 1 }
...
void getTests(std::vector<std::string> &tests, const char *name = "data",
              std::string indent = "") {
  DIR *dir;
  struct dirent *entry;
...
LightMatrix<T>::LightMatrix(LightMatrix<T> &&other)
    : nrows_(other.nrows_), ncols_(other.ncols_), data_(other.data_),
...
template <typename T> void LightMatrix<T>::transpose_in_place() {
...
template <typename T> void LightMatrix<T>::set_identity() {
...
template <typename T> void LightMatrix<T>::scale_col(int i, const T &val) {
  for (int j = 0; j < nrows_; j++)
    (*this)(j, i) *= val;
}

template <typename T> void LightMatrix<T>::scale_row(int i, const T &val) {
  for (int j = 0; j < ncols_; j++)
    (*this)(i, j) *= val;
}

template <typename T> void LightMatrix<T>::add(const LightMatrix<T> &other) {
  for (int i = 0; i < size(); ++i)
    data_[i] += other.data_[i];
}

template <typename T> void LightMatrix<T>::set(const T *const data) {
  for (int j = 0; j < size(); j++)
    data_[j] = data[j];
}

template <typename T> void LightMatrix<T>::set_row(int i, const T &val) {
  for (int j = 0; j < ncols_; j++)
    (*this)(i, j) = val;
}

template <typename T> void LightMatrix<T>::set_col(int i, const T &val) {
  for (int j = 0; j < nrows_; j++)
    (*this)(j, i) = val;
}

template <typename T> void LightMatrix<T>::set_col(int i, const T *const data) {
  for (int j = 0; j < nrows_; j++)
    (*this)(j, i) = data[j];
}

template <typename T> void LightMatrix<T>::fill(const T &val) {
  for (int i = 0; i < size(); i++)
    data_[i] = val;
}

template <typename T> void LightMatrix<T>::resize(int nrows, int ncols) {
