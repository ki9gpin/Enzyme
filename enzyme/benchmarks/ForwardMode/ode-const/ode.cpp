...
double foobar(double t, uint64_t iters) {
  ...
  // typedef controlled_runge_kutta< runge_kutta_dopri5< state_type , typename
  // state_type::value_type , state_type , double > > stepper_type;
  typedef euler<state_type, typename state_type::value_type, state_type, double>
      stepper_type;
...
int main(int argc, char **argv) {
  ...
  unsigned i = 0;
  for (int iters = max_iters / 20; iters <= max_iters;
       iters += max_iters / 20) {
    printf("iters=%d\n", iters);
