...
adouble afoobar(adouble t, uint64_t iters) {
  ...
  // typedef controlled_runge_kutta< runge_kutta_dopri5< astate_type , typename
  // astate_type::value_type , astate_type , adouble > > stepper_type;
  typedef euler<astate_type, typename astate_type::value_type, astate_type,
                adouble>
      stepper_type;
...
static double afoobar_and_gradient(double xin, double &xgrad, uint64_t iters) {
  adept::Stack stack;
  adouble x = xin;
  stack.new_recording();
